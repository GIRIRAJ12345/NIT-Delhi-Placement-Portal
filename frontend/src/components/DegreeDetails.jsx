...
const DegreeDetails = ({ degreeDetails }) => {
    return (
        <div>
            ...
            <h2 className='my-3 font-semibold text-gray-500'>{degreeDetails.degreeDescription}</h2>

            <ul className='pl-6 list-disc text-gray-700 my-3'>
                {
                    degreeDetails.courses.map((courseInfo) => {
                        return (<li key={courseInfo.id}>{courseInfo.course} <br /> {courseInfo?.specialization}</li>);
                    })
                }
            </ul>
